...
const EntryForm: React.FC<EntryFormProps> = ({ onSubmit, onCancel }) => {
  ...
  const handleInputChange = (field: keyof EntryFormData, value: any) => {
    setFormData((prev) => {
      ...
      if (field === "grossWeight" || field === "tare" || field === "quantity") {
        const grossWeight = Number(newData.grossWeight) || 0;
        const tare = Number(newData.tare) || 0;
        const quantity = Number(newData.quantity) || 0;

        newData.totalTare = tare * quantity;
        newData.netWeight = grossWeight - newData.totalTare;

        // Recalculate total value if unit value exists
